...
class FBDirMap (object):
    ...
    def Add(self,pSourceDir,pTargetDir):
        ...
        pSourceDir : The source directory 
        pTargetDir : The target directory 
        """
        pass
...
class FBPropertyViewManager (FBComponent):
    ...
    Interface to create new property views. There are two ways of creating properties view:on library load using AddPropertyView, RemovePropertyView, HidePropertyView - example can be found in while application is running using FBPropertyViewList - example can be found in .pySee sample: PropertyViewManager.py.     
    """
...
class FBAnimationNode (FBComponent):
    """
       <a href='ms-its:MotionBuilder_SDK_Samples.chm::/Scripts/Samples/FCurve/CopyAnimation.html'>See samples: CopyAnimation.py,</a> ClearKeysOnSelectedModels.py, TraversingRelationConstraint.py, FCurveEditor.py.     
...
See samples: ExportAnimationLibrary.py, GoToNextTake.py, GoToPreviousTake.py, MirrorPoseOverTime.py, MultiLayerKeying.py, RenameFirstTakeOnMultipleFiles.py, SaveOneTakePerFile.py, TimeCodeKeying.py.     
    """
...
class FBTexture (FBBox):
    """
       <a href='ms-its:MotionBuilder_SDK_Samples.chm::/Scripts/Samples/HUD/HUDElements.html'>See samples: HUDElements.py,</a> MaterialAndTexture.py, TextureAnimation.py, VideoInput.py, VideoMemory.py, DeleteUnusedMedia.py.     
...
class FBProgress (FBVisualComponent):
    ...
    See samples: 3dsMaxBipedTemplate.py, MirrorPoseOverTime.py, FBProgress.py. A property is a holder for function callbacks into the internalsof the application.You cannot instantiate FBProperty objects. To reference a property: Use an instance of an FBComponent object. The methods FBComponent::PropertyCreate and FBComponent::PropertyRemove can be used to modify an object's set of properties. When accessing a FBProperty object via its containing object, you can get or set (assuming it is not read-only) its value directly, for example in Python: myObject.Visibility = True. FBPropertyManager exists in all FBComponent objects, and contains an array of all the registered properties. Use FBProperty::Find to find a property by name. When accessing a property reference directly, its value is obtained via its 'Data' member. 
@code
...
class FBTool (FBLayout):
    """
    Tool class.     
    See samples: MBFileRefDemo.py, CloseTool.py, SafeToolCreationExample.py, ToolCommunicationReceiver.py, ToolNativeWidgetHolder.py.     
    """
